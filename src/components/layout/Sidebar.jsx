...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const chartRoutes = ['/bar-chart', '/line-chart', '/pie-chart', '/heatmap', '/scatter-chart', '/area-chart', '/gauge-chart', '/tree-chart'];
    const reportRoutes = ['/reports/sales', '/reports/users', '/reports/performance', '/reports/financial', '/reports/marketing', '/reports/custom'];
    const helpRoutes = ['/help/user-guide', '/help/faq', '/help/api'];
    
    if (chartRoutes.includes(location.pathname)) {
      setExpandedItem('Visualizations');
    } else if (reportRoutes.includes(location.pathname)) {
      setExpandedItem('Reports');
    } else if (helpRoutes.includes(location.pathname)) {
      setExpandedItem('Help & Documentation');
    }
  }, [location.pathname]);
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-16 left-0 bottom-0 z-30 w-64 bg-white dark:bg-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          {/* Navigation */}
          <nav className="flex-1 px-4 pt-2 pb-4 space-y-2 overflow-y-auto">
            {navigation.map((item) => (
              <div key={item.name}>
                {item.children ? (
                  <div>
